...
namespace DispatchService.Application.DispatchAssignment
{
    public class Create
    {
        ...
        public class CommandValidator : AbstractValidator<Command>
        {
            ...
            {
                RuleFor(x => x.DispatchOrderId)
                    .NotEmpty().WithMessage("DispatchOrderId is required.");
        ...
        public class Handler : IRequestHandler<Command, DispatchAssignmentDTO>
        {
            ...
            public async Task<DispatchAssignmentDTO> Handle(Command request, CancellationToken ct)
            {
                ...
                var dto = new CreateDispatchAssignmentRequestDTO
                {
                    UnitId = request.UnitId
                };

                return await _dispatchSvc.AssignUnitToDispatch(request.DispatchOrderId, dto, ct);
